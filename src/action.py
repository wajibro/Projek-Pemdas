...
def chance_card(self):
  ...
  if self.which_player_loc in [16, 26, 38]:
    if hasattr(self, 'card_bg_item'):
      self.canvas.delete(self.card_bg_item)
      self.canvas.delete(self.card_text)
    daftar_kartu = ['memenangkan kompetisi Sains Data\n mendapat uang sebesar Rp 1.000.000', 
  ...
  if self.which_player_loc == 7:
    ...
    match roulet:
      ...
      case 3:
        ...
        self.amount_add_player(self.which_player, total)
      case 4:
        self.card_text = self.canvas.create_text(0, 250, anchor='nw', text=f'Hai {self.which_player_name}, kamu {daftar_kartu[roulet]}', fill='white', font=('Arial', 12, 'bold'))
        total = self.amount_read_player(self.which_player)
